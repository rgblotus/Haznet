...
async def get_public_stats(db: AsyncSession = Depends(get_db)):
    ...
    completed_result = await db.execute(
        select(func.count(Requisition.id)).where(Requisition.status == RequisitionStatus.COMPLETED)
    )
    completed_requisitions = completed_result.scalar() or 0
    
    # Active tenders
    tender_result = await db.execute(
        select(func.count(Tender.id)).where(Tender.status.in_([TenderStatus.BIDDING, TenderStatus.PUBLISHED, TenderStatus.EVALUATING]))
    )
    ...
    order_result = await db.execute(
        select(func.count(Order.id)).where(Order.status.in_([OrderStatus.APPROVED, OrderStatus.ISSUED, OrderStatus.SHIPPED]))
    )
