...
class User(Base):
    ...
    role = Column(
        SAEnum(
            UserRole, name="user_role", values_callable=lambda x: [i.value for i in x]
        ),
        nullable=False,
    )
    ...
    requisitions_created = relationship(
        "Requisition", back_populates="creator", foreign_keys="Requisition.creator_id"
    )
    requisitions_owned = relationship(
        "Requisition",
        back_populates="current_owner",
        foreign_keys="Requisition.current_owner_id",
    )
...
class Requisition(Base):
    ...
    priority = Column(
        SAEnum(
            Priority, name="priority", values_callable=lambda x: [i.value for i in x]
        ),
    ...
    # Workflow tracking
    hodi_cnp_approval = Column(
        SAEnum(
            HODCNPApproval,
            name="hodi_cnp_approval",
            values_callable=lambda x: [i.value for i in x],
        ),
        default=HODCNPApproval.PENDING,
    )
    inventory_check_status = Column(
        SAEnum(
            InventoryCheckStatus,
            name="inventory_check_status",
            values_callable=lambda x: [i.value for i in x],
        ),
        default=InventoryCheckStatus.NOT_CHECKED,
    )
    ...
    creator = relationship(
        "User", back_populates="requisitions_created", foreign_keys=[creator_id]
    )
    current_owner = relationship(
        "User", back_populates="requisitions_owned", foreign_keys=[current_owner_id]
    )
    department = relationship("Department")
    tender = relationship(
        "Tender", back_populates="requisition", foreign_keys="Tender.requisition_id"
    )
    messages = relationship("Message", back_populates="requisition")
    orders = relationship("Order", back_populates="requisition")
    documents = relationship(
        "Document", back_populates="requisition", cascade="all, delete-orphan"
    )
...
class Document(Base):
    ...
    requisition_id = Column(
        UUID(as_uuid=True), ForeignKey("requisitions.id"), nullable=False, index=True
    )
    tender_id = Column(
...
class Tender(Base):
    ...
    requisition = relationship(
        "Requisition", back_populates="tender", foreign_keys=[requisition_id]
    )
...
class PostOrder(Base):
    ...
    status = Column(
        ...
        default=PostOrderStatus.PENDING_INSPECTION,
    )
...
class RequisitionStageHistory(Base):
    ...
    status = Column(
        ...
        default=InternalApprovalStatus.PENDING,
    )
...
class InternalApprovalDetail(Base):
    ...
    tender_committee_recommended_by = Column(
        UUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
...
class BidEvaluationDetail(Base):
    ...
    tender_evaluation_id = Column(
        UUID(as_uuid=True), ForeignKey("tender_evaluation.id"), nullable=True
    )

    technical_score = Column(Float, nullable=True)
    technical_evaluated_at = Column(DateTime(timezone=True), nullable=True)
    technical_evaluated_by = Column(
        UUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
    ...
    commercial_evaluated_by = Column(
        UUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
    ...
    bid = relationship("Bid", foreign_keys=[bid_id])
    tender_evaluation = relationship(
        "TenderEvaluation", foreign_keys=[tender_evaluation_id]
    )
...
class TenderCancellation(Base):
    ...
    reason = Column(
        SAEnum(
            TenderCancellationReason,
            name="tender_cancellation_reason",
            values_callable=lambda x: [i.value for i in x],
        ),
        nullable=False,
    )
